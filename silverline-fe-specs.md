...
- **The DP-13 bit table is unsourced, and a real FI 70 contradicts it at bit 6.** Section 6 and the bit map this doc carries are reconstructed from sibling schemas, not from a Poolstar document — `FAULT_BIT_NAMES`/`FAULT_BIT_CODES` inherited that provenance. First hardware readout from a classic 5-DP v3.3 unit to disagree: a Poolex Silverline FI 70 / PC-SLP070N read `DP 13 = 64` (bit 6 alone) while its own wired controller displayed **Er10** (issue #21, @TRIAG73, 2026-09-13), where the table names bit 6 the inlet-water sensor and prints P3. One readout cannot re-shift a table seven profiles decode against, and a display code is an observation rather than a decode — so the FI 70 got its own layout (`LAYOUT_SLP070` / `SLP070_FAULT_TABLE`) that carries every other bit over unchanged and leaves bit 6 unnamed and uncoded. It surfaces as the raw `bit6`: no binary sensor, no Repair card. Treat the remaining classic bits as unconfirmed rather than verified.
